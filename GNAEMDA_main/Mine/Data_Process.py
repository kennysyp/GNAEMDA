...
import warnings
from sklearn.metrics import average_precision_score, roc_auc_score, adjusted_mutual_info_score, precision_recall_curve
...
def mask_test_edges(adj, test_percent=20.):#, val_percent=1.
    ...
    train_edges = np.delete(edges_positive, np.hstack([test_edge_idx]), axis = 0) # positive train edges
    ...
    return adj_train, 'none', val_edges_false, test_edges, test_edges_false
...
def get_roc_score(edges_pos, edges_neg, emb):
    ...
    # Computes metrics
    roc_score, fpr, tpr= roc_auc_score(labels_all, preds_all)
    # prc_score = precision_recall_curve(labels_all.preds_all)
